...
import { Moon, Sun, LayoutDashboard, Settings, Calendar, Clock, Calculator } from 'lucide-react';
...
export function Layout() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 font-sans text-slate-900 dark:text-slate-50 flex flex-col">
      <nav className="sticky top-0 z-40 w-full backdrop-blur-xl bg-white/70 dark:bg-slate-950/70 border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <Link to="/" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            ...
            <span className="font-bold text-xl tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-violet-600 to-indigo-600 dark:from-violet-400 dark:to-indigo-400 hidden sm:block">
              SmartAttend
            </span>
          </Link>
            ...
            <div className="hidden md:flex items-center gap-4 mr-4">
              {navLinks.map((link) => (
                <Link 
                  key={link.to}
                  to={link.to} 
                  className={`text-sm font-medium transition-colors hover:text-violet-600 dark:hover:text-violet-400 ${location.pathname === link.to ? 'text-violet-600 dark:text-violet-400' : 'text-slate-500'}`}
                >
                  {link.label}
                </Link>
              ))}
      ...
      <main className="flex-1 max-w-6xl mx-auto w-full p-4 md:p-8 animate-in fade-in slide-in-from-bottom-4 duration-500 flex flex-col justify-between">
      ...
      <div className="fixed bottom-0 left-0 w-full md:hidden bg-white dark:bg-slate-950 border-t border-slate-200 dark:border-slate-800 pb-safe z-40">
        <div className="flex justify-around p-2">
          {navLinks.map((link) => {
            ...
            return (
              <Link key={link.to} to={link.to} className={`flex flex-col items-center gap-1 p-1 ${location.pathname === link.to ? 'text-violet-600 dark:text-violet-400' : 'text-slate-500'}`}>
